...
export class PlanCreateComponent implements OnInit {
  ...
  buildPlanObject(status: PlanStatus = 'Published'): Plan {
    ...
    return {
      ...
      name: (val.name || '').trim() || (status === 'Draft' ? 'Untitled Draft Plan' : 'New Learning Plan'),
      ...
      durationType: (val.durationType as DurationType) || 'Yearly',
      startDate: (val.startDate || '').trim() || '01/01/2026',
      endDate: (val.endDate || '').trim() || '31/12/2026',
      enrollmentType: (val.enrollment?.mode as EnrollmentType) || (val.enrollmentType as EnrollmentType) || 'Open',
      recurringPlan: val.recurring?.enabled ? `Yes (${val.recurring?.cycleConfig || 'Annual Cycle'})` : null,
      status: this.isEditing() ? (existing?.status || status) : (status || 'Draft'),
