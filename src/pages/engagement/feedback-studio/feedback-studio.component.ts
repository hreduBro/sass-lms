...
@Component({
    ...
    template: `
        <div class="space-y-6">
            ...
            <div
                    class="p-6 rounded-3xl bg-base-100 border border-base-300 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <div class="flex items-center gap-2">
            <span
                    class="px-2.5 py-0.5 rounded-full text-[11px] font-bold bg-tenant-50 dark:bg-tenant-900/40 text-tenant-700 dark:text-tenant-300 border border-tenant-500/20">
              One per Plan • Immutable Versioning Engine
            </span>
                        <span
                                class="text-xs text-text-secondary">• Active Version: <strong>{{ activeForm()?.versions?.slice(-1)?.[0]?.versionLabel || 'v1' }}</strong></span>
                    </div>
                ...
                <!-- Mode / Sub-Tabs -->
                <div
                        class="flex items-center gap-1.5 p-1 bg-base-200 rounded-2xl border border-base-300 self-start md:self-auto">
                    <button
                            type="button"
            ...
            @if (activeSubTab() === 'responses') {
                <div class="space-y-4 animate-fade-in">
                    ...
                    <div class="space-y-3 relative z-30">
                        ...
                        <div
                                class="bg-white dark:bg-base-100 rounded-3xl border border-slate-200/80 dark:border-base-300 p-3.5 sm:p-4 shadow-2xs flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">

                            <!-- Search Bar with Integrated Action Buttons -->
                            <div class="flex items-center gap-3 flex-1 max-w-2xl">
                                <div class="relative flex-1">
                  <span
                          class="material-symbols-outlined text-slate-400 absolute left-3.5 top-1/2 -translate-y-1/2 text-lg select-none pointer-events-none">
                    search
                  </span>
                                    <input
                                            type="text"
                  ...
                  <span class="material-symbols-outlined text-base"
                        [class.text-white]="isFilterPanelOpen()">filter_list</span>
                                    <span>Filters</span>
                                </button>
                ...
                <span
                        class="px-3 py-1.5 rounded-xl bg-slate-50 dark:bg-base-200 border border-slate-200/80 dark:border-base-300 font-mono font-semibold text-xs text-text-secondary">
                  Showing <strong class="text-text-primary">{{ filteredResponses().length }}</strong> of <strong
                        class="text-text-primary">{{ planResponses().length }}</strong> submissions
                </span>
                            </div>
                        ...
                        @if (isFilterPanelOpen()) {
                            <div
                                    class="bg-white dark:bg-base-100 rounded-2xl border border-slate-200/80 dark:border-base-300 p-4 sm:p-5 shadow-sm animate-in fade-in slide-in-from-top-2 duration-200 space-y-4">
                    ...
                    Combine criteria with AND &bull; Multiple values in same category with OR
                                    ...
                                    <div class="space-y-2.5">
                                        ...
                                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                            @for (ver of versionFilterOptions(); track ver.value) {
                                                <label
                                                        class="flex items-center gap-2 text-xs text-text-primary cursor-pointer group select-none p-1 rounded-lg hover:bg-slate-50 dark:hover:bg-base-200 transition-colors">
                                                    <input
                                                            type="checkbox"
                                                    ...
                                                    <span
                                                            class="px-2.5 py-1 rounded-lg text-xs font-semibold border transition-all inline-flex items-center gap-1.5 shadow-2xs truncate"
                                                            [class]="ver.badgeClass">
                        ...
                        @if (hasActiveFilters() || searchQuery()) {
                            <div class="flex items-center flex-wrap gap-2 pt-1 animate-in fade-in">
                                ...
                                @if (searchQuery()) {
                                    <span
                                            class="inline-flex items-center gap-1 px-2.5 py-1 bg-slate-100 dark:bg-base-200 text-slate-700 dark:text-slate-200 border border-slate-200 dark:border-base-300 text-xs rounded-xl font-medium">
                    <span>Query: "{{ searchQuery() }}"</span>
                    <button type="button" (click)="onSearchChange('')"
                            class="hover:text-rose-500 font-bold ml-1 cursor-pointer border-0 bg-transparent">✕</button>
                  </span>
                                }
                    ...
                    <button type="button" (click)="removeVersionFilter(vId)"
                            class="hover:text-rose-500 font-bold ml-1 cursor-pointer border-0 bg-transparent">✕</button>
                  </span>
                                }
                    ...
                    <button type="button" (click)="removePrivacyFilter(priv)"
                            class="hover:text-rose-500 font-bold ml-1 cursor-pointer border-0 bg-transparent">✕</button>
                  </span>
                                }
                            </div>
                    ...
                    @if (planResponses().length === 0) {
                        ...
                        <div
                                class="p-12 text-center rounded-2xl bg-base-100 dark:bg-base-200 border border-base-300 dark:border-slate-800">
                            <span class="material-symbols-outlined text-4xl text-text-secondary/40">description</span>
                            <p class="text-xs font-bold text-text-primary mt-2">No response information is available to
                                display.</p>
                            <p class="text-[11px] text-text-secondary mt-1">Once trainees complete this plan or phase,
                                their feedback
                                will appear here in real-time.</p>
                            <button
                                    type="button"
                                    (click)="activeSubTab.set('submit-simulator')"
                        ...
                        <div
                            ...
                            <p class="text-xs font-bold text-text-primary mt-2">No response found matching the selected
                                filters.</p>
                            <p class="text-[11px] text-text-secondary mt-1">Try changing the version or clearing your
                                keyword
                                filter.</p>
                            <button
                                    type="button"
                        ...
                        <div
                                class="rounded-2xl border border-base-300 dark:border-slate-800 bg-base-100 dark:bg-base-200 overflow-hidden shadow-sm">
                                                ...
                                                <td class="py-3 px-4">
                                                    @if (resp.isAnonymous) {
                                                        <div class="flex items-center gap-2.5">
                                                            <div
                                                                    class="w-8 h-8 rounded-full bg-slate-200 dark:bg-slate-700 text-slate-500 flex items-center justify-center">
                                                        ...
                                                        <div class="flex items-center gap-2.5">
                                                            <app-custom-avatar [name]="resp.traineeName || 'Trainee'"
                                                                               [url]="resp.traineeAvatar"
                                                                               size="sm"
                                                                               shape="squircle"></app-custom-avatar>
                                                            <div>
                                                                <span class="font-bold text-text-primary">{{ resp.traineeName }}</span>
                                                                <span
                                                                        class="text-[10px] text-text-secondary block">{{ resp.traineeEmail || 'trainee@brac.net' }}</span>
                                                            </div>
                                                        </div>
                                                    }
                                                </td>
                          ...
                          <span
                                  class="px-2 py-0.5 rounded text-[11px] font-mono font-bold bg-tenant-100 dark:bg-tenant-900/40 text-tenant-700 dark:text-tenant-300">
                            {{ resp.versionLabel }}
                          </span>
                                                </td>
                          ...
                          <span
                                  class="px-2 py-0.5 rounded-full text-[11px] font-semibold bg-emerald-100 text-emerald-800 dark:bg-emerald-950/40 dark:text-emerald-300">
            ...
            @if (activeSubTab() === 'versions') {
                <div class="space-y-6 animate-fade-in">
                    ...
                    <div
                            class="p-5 rounded-2xl bg-base-100 dark:bg-base-200 border border-base-300 dark:border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        <div>
                            <h3 class="text-sm font-bold text-text-primary">Immutable Version History</h3>
                            <p class="text-xs text-text-secondary mt-0.5">
                                Modifying questions forks a new version (copy-on-edit). All prior responses remain
                                permanently anchored
                                to their original questionnaire.
                            </p>
                        </div>
                        <button
                                type="button"
                            ...
                            <div
                                ...
                                <div>
                                    <div class="flex items-center justify-between gap-2">
                                        <div class="flex items-center gap-2">
                      <span
                              class="px-2.5 py-1 rounded-md text-xs font-mono font-bold bg-tenant-100 dark:bg-tenant-900/40 text-tenant-700 dark:text-tenant-300">
                        {{ ver.versionLabel }}
                      </span>
                                            <span
                    ...
                    <span class="text-[10px] uppercase font-bold text-text-secondary tracking-wider block">Question Structure ({{ ver.questions.length }}
                        items):</span>
                        ...
                        <span class="material-symbols-outlined text-xs text-text-secondary">
                          {{ q.type === 'text' ? 'short_text' : (q.type === 'singleSelect' ? 'radio_button_checked' : 'check_box') }}
                                                ...
                                                <span class="font-medium text-text-secondary truncate max-w-[280px]">Q{{ q.order }}
                                                    : {{ q.text }}</span>
                                            </div>
                                        }
                                    </div>
                                ...
                                <div
                                        class="mt-4 pt-3 border-t border-base-300/60 dark:border-slate-800/60 flex items-center justify-between text-xs">
            ...
            @if (activeSubTab() === 'submit-simulator') {
                <div class="max-w-2xl mx-auto space-y-6 animate-fade-in">
                    ...
                    <div
                            class="p-6 rounded-2xl bg-base-100 dark:bg-base-200 border border-base-300 dark:border-slate-800 shadow-sm">
                        <div class="flex items-center justify-between gap-4 pb-4 border-b border-base-300 dark:border-slate-800">
                            <div>
                                ...
                                <p class="text-xs text-text-secondary mt-0.5">Answering against version
                                    <strong>{{ currentPublishedVersion()?.versionLabel }}</strong></p>
                            </div>
                        ...
                        <form (ngSubmit)="submitTraineeFeedback()" class="mt-6 space-y-6 text-xs">
                            ...
                            @for (q of currentPublishedVersion()?.questions || []; track q.questionId; let i = $index) {
                                <div
                                        class="p-4 rounded-xl bg-base-200/50 dark:bg-base-300/30 border border-base-300 dark:border-slate-800 space-y-3">
                                    <div class="flex items-start justify-between gap-2">
                                        ...
                                        <span
                                                class="text-[10px] font-semibold text-text-secondary uppercase px-1.5 py-0.5 bg-base-100 dark:bg-base-200 rounded">
                                    ...
                                    @if (q.type === 'singleSelect') {
                                        <div class="space-y-2">
                                            @for (opt of q.options || []; track opt.optionId) {
                                                <label
                                                        class="flex items-center gap-2.5 p-2.5 rounded-xl bg-base-100 dark:bg-base-200 border border-base-300/60 dark:border-slate-800 hover:border-tenant-500 transition-colors cursor-pointer">
                                                    <input
                                                            type="radio"
            ...
            @if (inspectingResponse()) {
                <div
                    ...
                    <div
                        ...
                        <div
                                class="p-5 border-b border-base-300 dark:border-slate-800 flex items-center justify-between bg-base-200/50 dark:bg-base-300/30">
                            <div>
                                <div class="flex items-center gap-2">
                  <span
                          class="px-2 py-0.5 rounded text-[10px] font-mono font-bold bg-tenant-100 dark:bg-tenant-900/40 text-tenant-700 dark:text-tenant-300">
                    Form Version: {{ inspectingResponse()!.versionLabel }}
                                ...
                                <h3 class="text-sm font-bold text-text-primary mt-1">
                                    Submission
                                    from {{ inspectingResponse()!.isAnonymous ? 'Anonymous Trainee' : inspectingResponse()!.traineeName }}
                                </h3>
                            </div>
                            <button
                                    type="button"
                        ...
                        <div class="p-6 overflow-y-auto space-y-4 text-xs">
                            @for (item of inspectedVersionQuestions(); track item.questionId; let idx = $index) {
                                <div
                                        class="p-4 rounded-xl bg-base-200/50 dark:bg-base-300/30 border border-base-300 dark:border-slate-800">
                                    <h4 class="font-bold text-text-primary leading-snug">Q{{ idx + 1 }}
                                        : {{ item.text }}</h4>
                ...
                <div
                    ...
                    <div
                        ...
                        <div
                            ...
                            <div class="flex items-center gap-2.5">
                                ...
                                <div>
                                    ...
                                    <p class="text-[11px] text-text-secondary">Copy-on-edit architecture protects
                                        historical
                                        responses.</p>
                                </div>
                            </div>
                        ...
                        <div class="p-6 overflow-y-auto space-y-6 text-xs flex-1">
                            ...
                            <div
                                    class="p-4 rounded-xl bg-base-200/60 dark:bg-base-300/40 border border-base-300 dark:border-slate-800">
                                <label class="font-bold text-text-primary block mb-1">Version Release Notes / Change
                                    Summary</label>
                                <input
                                        type="text"
                        ...
                        <div
                                class="p-4 border-t border-base-300 dark:border-slate-800 bg-base-200/50 flex items-center justify-between">
              <span class="text-[11px] text-text-secondary">
