...
@Component({
  selector: 'app-org-add-widget-modal',
      ...
      <div 
        ...
        <div class="px-6 py-5 border-b border-base-300 bg-base-200/40 flex items-center justify-between flex-shrink-0">
          <div class="flex items-center gap-3">
            ...
            <div>
              <h2 class="text-base font-bold text-text-primary">Organization Widget Catalog</h2>
              <p class="text-xs text-text-secondary">Select an interactive component to add to your custom organization canvas</p>
            </div>
          </div>
          <button 
            type="button"
        ...
        <div class="px-6 py-3.5 border-b border-base-300/80 bg-base-100 space-y-3 flex-shrink-0">
          ...
          <div class="relative">
            ...
            <input 
              ...
              placeholder="Search components by keyword, metric, or purpose..."
...
export class OrgAddWidgetModalComponent {
  close = output<void>();
  selectWidget = output<OrgDashboardWidget>();

  catalog = ORG_WIDGET_CATALOG;
  ...
  categories = [
    ...
    { id: 'directory', label: 'Directory', icon: 'folder_shared' }
  ];
  ...
  addWidget(item: OrgWidgetCatalogItem) {
    const newWidget: OrgDashboardWidget = {
