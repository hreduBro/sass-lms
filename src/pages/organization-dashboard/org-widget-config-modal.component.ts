    ...
    <div 
      ...
      <div 
        ...
        <div class="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
              ...
              <div class="grid grid-cols-4 gap-1">
                @for (span of [1, 2, 3, 4]; track span) {
                  <button 
                    ...
                    [class]="formData.colSpan === span ? 'bg-tenant-500 text-white border-transparent shadow-xs' : 'bg-base-200 text-text-secondary border-base-300 hover:bg-base-300'">
                    {{ span * 25 }}%
                  </button>
                }
              </div>
              ...
              <div class="grid grid-cols-4 gap-1">
                @for (rSpan of [1, 2, 3, 4]; track rSpan) {
                  <button 
                    ...
                    class="py-1.5 rounded-lg text-xs font-semibold border transition-all cursor-pointer"
                    [class]="(formData.rowSpan || 2) === rSpan ? 'bg-tenant-500 text-white border-transparent shadow-xs' : 'bg-base-200 text-text-secondary border-base-300 hover:bg-base-300'">
                    {{ rSpan }}x
                  </button>
                }
              </div>
          ...
          @if (formData.type === 'org_broadcast_banner') {
            <div class="pt-2 border-t border-base-300 space-y-3">
              ...
              <textarea 
                ...
                rows="3"
                class="w-full px-3.5 py-2 text-xs rounded-xl bg-base-200 border border-base-300 text-text-primary focus:outline-none focus:border-tenant-500 transition-colors"
                placeholder="Enter system-wide announcement message..."></textarea>
          ...
          @if (formData.type === 'org_status_breakdown') {
            <div class="pt-2 border-t border-base-300 space-y-2">
              <label class="block text-xs font-semibold text-text-primary">Chart Presentation Style</label>
              <div class="grid grid-cols-2 gap-2">
                <button 
                  type="button"
                  (click)="formData.config.chartType = 'donut'"
                  class="py-2 px-3 rounded-xl text-xs font-semibold flex items-center justify-center gap-1.5 border transition-all cursor-pointer"
                  [class]="formData.config.chartType !== 'bar' ? 'bg-tenant-500 text-white border-transparent' : 'bg-base-200 text-text-secondary border-base-300'">
                  <span class="material-symbols-outlined text-sm">donut_large</span>
                  <span>Donut Breakdown</span>
                </button>
                <button 
                  type="button"
                  (click)="formData.config.chartType = 'bar'"
                  class="py-2 px-3 rounded-xl text-xs font-semibold flex items-center justify-center gap-1.5 border transition-all cursor-pointer"
                  [class]="formData.config.chartType === 'bar' ? 'bg-tenant-500 text-white border-transparent' : 'bg-base-200 text-text-secondary border-base-300'">
...
export class OrgWidgetConfigModalComponent {
